...
const Rules = (props:TabsSection&{lang:string}) => {
  return (
    ...
    <div className='grid grid-cols-1 lg:grid-cols-2 gap-10'>
      {props.items.map(item=><RuleCard points={item.points.map(item=>localeText(item, props.lang))} heading={localeText(item.title,props.lang)} subheading={localeText(item.subtitle,props.lang)} color="bg-[#004AAD]" />)}
      </div>
    </div>
  )
}
